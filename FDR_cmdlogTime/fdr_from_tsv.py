...
def main():
    (
        ...
        my_args,
        addl_logfile,
    ) = cmdlogtime.begin(COMMAND_LINE_DEF_FILE)
    ...
    if rows_to_keep_f.endswith("ALLZZZ"):
        ...
        df_rows = pd.read_csv(
            rows_to_keep_f, sep="\t", index_col=0
        )  # header=None)
    ...
    df_to_use["fdr2"] = multitest.multipletests(
        df_to_use[p_value_header].to_numpy(), method="fdr_bh"
    )[1]
